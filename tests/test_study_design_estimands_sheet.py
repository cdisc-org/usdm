...
def test_read_cell_by_name_error(mocker, globals):
  ...
  mock_parameters_correct(mea, [mocker.call('studyDesignEstimands', 1, -1, "Error attempting to read cell 'populationSubset'. Exception: Failed to detect column(s) 'populationSubset' in sheet", 40), 
                                mocker.call('studyDesignEstimands', None, None, "Unable to find population or cohort with name ''", 40),
                                mocker.call('studyDesignEstimands', None, None, "Failed to add IntercurrentEvent, no Estimand set", 40),
                               ])
