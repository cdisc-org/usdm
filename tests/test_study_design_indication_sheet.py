...
def test_create(mocker):
  ...
  mock_id.side_effect=['IndicationId_1', 'IndicationId_2', 'IndicationId_3']
  ...
  data = [['Indication 1', 'Indication One', 'Label One', 'Sponsor:X=Y'], ['Indication 2', 'Indication Two', '', 'SPONSOR: AAA=BBB'], ['Indication 3', 'Indication Three', '', 'SPONSOR: WWW=1234, SPONSOR: EEE=3456']]
...
def test_read_cell_by_name_error(mocker):
  ...
  mock_read = mocker.patch("pandas.read_excel")
  mock_read.return_value = pd.DataFrame(data, columns=['studyIndicationName', 'studyIndicationDescription'])
  Indications = StudyDesignIndicationSheet("")
  mock_error.assert_called()
  assert mock_error.call_args[0][0] == "studyDesignIndications"
  assert mock_error.call_args[0][1] == None
  assert mock_error.call_args[0][2] == None
  assert mock_error.call_args[0][3] == "Exception [Failed to detect column(s) 'studyIndicationType, type' in sheet] raised reading sheet."
