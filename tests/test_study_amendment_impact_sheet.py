...
def test_create(mocker, globals):
  ...
  object_data = [
    {'cls': StudyAmendmentImpact, 'data': {'text': sheet_data['text'][0], 'isSubstantial': sheet_data['substantial'][0], 'type': expected_1}},
    {'cls': StudyAmendmentImpact, 'data': {'text': sheet_data['text'][1], 'isSubstantial': sheet_data['substantial'][1], 'type': expected_2}},
    {'cls': StudyAmendmentImpact, 'data': {'text': sheet_data['text'][2], 'isSubstantial': sheet_data['substantial'][2], 'type': expected_3}}
  ]
...
def test_read_cell_by_name_error(mocker, globals):
  ...
  mock_parameters_correct(mea, [mocker.call('amendmentImpact', 1, -1, "Error attempting to read cell 'text'. Exception: Failed to detect column(s) 'text' in sheet", 40), 
                                mocker.call('amendmentImpact', 1, 1, "Failed to find amendment with name 'A1'", 40)])
