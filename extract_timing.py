...
def save_csv(filename, tl, file_type, field_names, contents):
    with open(
        f"tests/integration_test_files/{filename}_{tl}_{file_type}.csv", "w", newline=""
    ) as f:
        if field_names:
...
timing_type_map = {"Before": "BEFORE", "After": "AFTER", "Fixed Reference": "FIXED"}
        ...
        for design in version["studyDesigns"]:
            ...
            for tl in design["scheduleTimelines"]:
                ...
                for instance in tl["instances"]:
                    instance_map[instance["id"]] = instance["id"].replace(
                        "ScheduledActivityInstance", "INSTANCE"
                    )
                ...
                for instance in tl["instances"]:
                    ...
                    if "encounterId" in instance:
                        encounter = (
                            encounter_map[instance["encounterId"]]
                            if instance["encounterId"] in encounter_map
                            else ""
                        )
                    ...
                    if instance["instanceType"] == "ScheduledDecisionInstance":
                        ...
                        for condition in instance["conditionAssignments"]:
                            conditions.append(
                                f"{condition[0]}: {instance_map[condition[1]]}"
                            )
                    ...
                    instances[4].append(
                        instance_map[instance["defaultConditionId"]]
                        if instance["defaultConditionId"] in instance_map
                        else "(EXIT)"
                    )
                    instances[5].append(conditions_str)
                    instances[6].append(
                        epoch_map[instance["epochId"]]
                        if instance["epochId"] in epoch_map
                        else ""
                    )
                ...
                for instance in tl["instances"]:
                    for timing in instance["timings"]:
                        ...
                        record = {
                            ...
                            if timing["relativeFromScheduledInstanceId"] in instance_map
                            else "",
                            "to": instance_map[timing["relativeToScheduledInstanceId"]]
                            if timing["relativeToScheduledInstanceId"] in instance_map
                            else "",
                            "timingValue": timing["value"],
                            "toFrom": to_from_map[timing["relativeToFrom"]["decode"]]
                            if the_type != "FIXED"
                            else "",
