...
class StudyDesignSpecimenRetentionSheet(BaseSheet):
  ...
  def _process_sheet(self):
    for index, row in self.sheet.iterrows():
      ...
      description = self.read_cell_by_name(index, 'description', default="", must_be_present=False)
      ...
      item: BiospecimenRetention = self.create_object(BiospecimenRetention, {'name': name, 'description': description, 'label': label, 'isRetained': retained, 'includesDNA': includesDNA})
      if item:
