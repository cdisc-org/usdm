        ...
        if match :
          ...
          if units_reqd:
            if parts['units']:
              ...
              self.errors.append(f"Units not found for the quantity but required '{quantity_info}'")
        else:
          self.errors.append(f"Could not decode the quantity value, not all required parts detected in '{quantity_info}'")
      elif not allow_empty:
        self.errors.append(f"Could not decode the quantity value, appears to be empty '{quantity_info}'")
      else:
        self.empty = True
    except Exception as e:
      self.errors.append(f"Exception [{e}] raised decoding range '{quantity_info}")
