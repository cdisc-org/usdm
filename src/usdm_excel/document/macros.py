...
class Macros():
    ...
    for ref in soup(['usdm:macro']):
      try:
        ...
        if self._valid_method(method):
          ...
          self._parent._general_error(f"Failed to resolve document macro '{attributes}', invalid method name {method}")
          ref.replace_with('Missing content: invalid method name')
      except Exception as e:
        self._parent._general_exception(f"Failed to resolve document macro '{attributes}'", e)
    ...
    if activity:
      ...
      for collection in [{'klass': BiomedicalConcept, 'ids': activity.biomedicalConceptIds, 'synonyms': True}, 
        ...
        for id in collection['ids']:
          next_bc = self._parent.globals.cross_references.get_by_id(collection['klass'], id)
          if next_bc:
  ...
  def _element(self, attributes, soup, ref) -> None:
    ...
    if self._elements.valid_method(method):
      ...
      self._parent._general_error(f"Failed to translate element method name '{method}' in '{attributes}', invalid method")
      ref.replace_with('Missing content: invalid method name')
  ...
  def _section(self, attributes, soup, ref) -> None:
    ...
    if instance.valid_method(method):
      ...
      self._parent._general_error(f"Failed to translate section method name '{method}' in '{attributes}', invalid method")
      ref.replace_with('Missing content: invalid method name')       
