...
class Activity():
  ...
  def _as_usdm(self):
    ...
    timelineId = ""
    if len(self._tls) > 0:
      timeline = self.managers.cross_references.get(ScheduleTimeline, self._tls[0])
      if timeline:
    ...
    for procedure in self._prs:
      ref = self.managers.cross_references.get(Procedure, procedure)
      if ref is not None:
    ...
    activity = self.managers.cross_references.get(Activity, self.name)
    if activity is None:
      try:
        ...
        self.managers.cross_references.add(self.name, activity)     
  ...
  def _to_bc_surrogates(self, name):
    return BiomedicalConceptSurrogate(
      id=self.managers.id_manager.build_id(BiomedicalConceptSurrogate),
      name=name,
      description=name,
      label=name,
      reference='None set'
