...
class SoAActivity():
  ...
  def __init__(self, parent: BaseSheet, row_index: int, map: dict):
    ...
    self._parent._debug(row_index, SoAColumnRows.BC_COL, f"Activity {self._parent_name}, {self._child_name} read. BC: {self._bcs}, PR: {self._prs}, TL: {self._tls}")
    self.activity = self._process_parent() if self._parent_name else self._process_child()
    
  def _process_parent(self) -> Activity:
    if self._child_name:
      self._parent._warning(self._row_index, SoAColumnRows.CHILD_ACTIVITY_COL, f"Both parent '{self._parent_name}' and child activity '{self._child_name}' found, child has been ignored")
    if self._parent_name in self._map:
      self._parent._error(self._row_index, SoAColumnRows.ACTIVITY_COL, f"Parent activity '{self._parent_name}' has already been referenced in the SoA, parent has been ignored")
      return None
    else:
  ...
  def _process_child(self) -> Activity:
    if self._child_name in self._map:
      self._parent._error(self._row_index, SoAColumnRows.CHILD_ACTIVITY_COL, f"Child activity '{self._child_name}' has already been referenced in the SoA, child has been ignored")
      return None
    else:
  ...
  def _set_activity(self, name, full_bc_items, surrogate_bc_items, procedures, timeline):
    activity = self._parent.globals.cross_references.get(Activity, name)
    if activity is None:
      ...
      activity = self._parent.create_object(Activity, params)
      if activity:
        self._parent.globals.cross_references.add(name, activity)     
        self._parent._warning(self._row_index, SoAColumnRows.BC_COL, f"No activity '{name}' found, so one has been created")
    else:
  ...
  def _set_biomedical_concepts(self):
    ...
    surrogate_bc_items = []
    for bc in self._bcs:
      if self._parent.globals.cdisc_bc_library.exists(bc):
        full_bc = self._parent.globals.cdisc_bc_library.usdm(bc)
        full_bc_items.append(full_bc.id)
        self.usdm_biomedical_concepts.append(full_bc)
        self._parent.globals.cross_references.add(full_bc.id, full_bc)
      else:
        params = {'name': bc, 'description': bc, 'label': bc, 'reference': 'None set'}
        item = self._parent.create_object(BiomedicalConceptSurrogate, params)
        if item:
          surrogate_bc_items.append(item.id)
          self.usdm_biomedical_concept_surrogates.append(item)
          self._parent.globals.cross_references.add(item.id, item)
    return full_bc_items, surrogate_bc_items
  
  def _set_procedures(self):
    results = []
    for procedure in self._prs:
      ref = self._parent.globals.cross_references.get(Procedure, procedure)
      if ref:
        results.append(ref)
      else:
        self._parent._warning(self._row_index, SoAColumnRows.BC_COL, f"No procedure '{procedure}' found, missing cross reference")
    return results
  
  def _set_timeline(self):
    result = None
    if self._tls:
      result = self._parent.globals.cross_references.get(ScheduleTimeline, self._tls[0])
      if not result:
        self._parent._warning(self._row_index, SoAColumnRows.BC_COL, f"No timeline '{self._tls[0]}' found, missing cross reference")
    return result
  ...
  def _get_observation_cell(self, row_index: int, col_index: int) -> tuple[list, list, list]:
    ...
    prs = []
    tls = []
    if not self._parent.cell_empty(row_index, col_index):
      value = self._parent.read_cell(row_index, col_index)
