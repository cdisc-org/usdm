...
class StudyDeviceRoleSheet(BaseSheet):
  ...
  def _process_applies_to_references(self, index):
    ...
    references = [x.strip() for x in self._state_split(self.read_cell_by_name(index, 'appliesTo'))]
      ...
      if reference:
        ...
        for klass in klasses:
          ...
          if xref:
            ...
            break
        if not found:
          self._error(index, self._get_column_index('appliesTo'), f"Could not resolve appliesTo reference '{reference}'")
    return results