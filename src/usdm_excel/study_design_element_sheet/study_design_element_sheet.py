  ...
  def __init__(self, file_path: str, globals: Globals):
    try:
      ...
      for index, row in self.sheet.iterrows():
        ...
        if start_rule_text:
          start_rule = TransitionRule(id=self.globals.id_manager.build_id(TransitionRule), name=f"ELEMENT_START_RULE_{index + 1}", text=start_rule_text)
        if end_rule_text:
          end_rule = TransitionRule(id=self.globals.id_manager.build_id(TransitionRule), name=f"ELEMENT_END_RULE_{index + 1}", text=end_rule_text)
        try:
          item = StudyElement(
