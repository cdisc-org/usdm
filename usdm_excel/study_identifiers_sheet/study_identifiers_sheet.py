...
class StudyIdentifiersSheet(BaseSheet):
  ...
  def study_registry(self):
    return CDISCCT(self.id_manager).code(code="C93453", decode="Study Registry")

  def study_sponsor(self):
    return CDISCCT(self.id_manager).code(code="C70793", decode="Clinical Study Sponsor")

  def regulatory(self):
    return CDISCCT(self.id_manager).code(code="C188863", decode="Regulatory Agency")
